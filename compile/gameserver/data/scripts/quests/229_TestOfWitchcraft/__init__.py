...
class Quest (JQuest) :
	...
	def onEvent (self,event,st) :
		...
		if event == "1":
			...
			st.takeItems(AKLANTOS_GEM6_ID,1)
                        st.set("cond","4")
			st.set("step","3")
			st.addSpawn(5101,70381, 109638, -3726)
		elif event == "7630_7" :
		...
		# Everts Events
			...
			st.addSpawn(5101,14027, 169896, -3646)
                        st.set("cond","9")
		return htmltext
	...
	def onTalk (self,npc,st):
		...
		id = st.getState()
		# Start the Quest, initialization and check if the player can take it
		if id == CREATED:
			for var in STATS:
				st.set(var,"0")
		...
		# in progress, player is working on the quest
			...
			if npcId == NPC[8]: # orim
				...
				if step == 13 and st.getQuestItemsCount(BRIMSTONE2_ID)==0:
					...
					htmltext = "7633-02.htm"
					st.addSpawn(5101,13631,169853,-3697)
					st.set("step","14")
				elif step == 15 :
	...
	def onKill (self,npc,player,isPet):
            st = player.getQuestState("229_TestOfWitchcraft")
            if st :
