...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,st):
   ...
   if npcId == 7629 :
     ...
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 :
     ...
     if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_WATER_ID)==0 and st.getQuestItemsCount(UNDINE_CHARM_ID)==0 :
        htmltext = "7413-01.htm"
        st.giveItems(UNDINE_CHARM_ID,1)
        st.set("cond","7")
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(UNDINE_CHARM_ID)==1 :
        if st.getQuestItemsCount(DAZZLING_DROP_ID) < 20 :
          ...
          st.playSound("ItemSound.quest_middle")
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_WATER_ID)==1 and st.getQuestItemsCount(UNDINE_CHARM_ID)==0 :
        htmltext = "7413-04.htm"
   elif npcId == 7411 :
     if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_FIRE_ID)==0 and st.getQuestItemsCount(SALAMANDER_CHARM_ID)==0 :
        htmltext = "7411-01.htm"
        st.giveItems(SALAMANDER_CHARM_ID,1)
        st.playSound("ItemSound.quest_middle")
        st.set("cond","7")
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(SALAMANDER_CHARM_ID)==1 :
        if st.getQuestItemsCount(FLAME_CRYSTAL_ID) < 5 :
          ...
          st.playSound("ItemSound.quest_middle")
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_FIRE_ID)==1 and st.getQuestItemsCount(SALAMANDER_CHARM_ID)==0 :
        htmltext = "7411-04.htm"
   elif npcId == 7412 :
     if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_WIND_ID)==0 and st.getQuestItemsCount(SYLPH_CHARM_ID)==0 :
        htmltext = "7412-01.htm"
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(SYLPH_CHARM_ID)==1 :
        if st.getQuestItemsCount(HARPYS_FEATHER_ID)+st.getQuestItemsCount(WYRMS_WINGBONE_ID)+st.getQuestItemsCount(WINDSUS_MANE_ID) < 40 :
          ...
          st.playSound("ItemSound.quest_middle")
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_WIND_ID)==1 and st.getQuestItemsCount(SYLPH_CHARM_ID)==0 :
        htmltext = "7412-05.htm"
   elif npcId == 7409 :
     if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_EARTH_ID)==0 and st.getQuestItemsCount(SERPENT_CHARM_ID)==0 :
        htmltext = "7409-01.htm"
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(SERPENT_CHARM_ID)==1 :
        if st.getQuestItemsCount(EN_MONSTEREYE_SHELL_ID)+st.getQuestItemsCount(EN_STONEGOLEM_POWDER_ID)+st.getQuestItemsCount(EN_IRONGOLEM_SCRAP_ID) < 30 :
          ...
          st.playSound("ItemSound.quest_middle")
     elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS_ID)==1 and st.getQuestItemsCount(TONE_OF_EARTH_ID)==1 and st.getQuestItemsCount(SERPENT_CHARM_ID)==0 :
        htmltext = "7409-06.htm"
   return htmltext
                             
 def onKill (self,npc,player,isPet):
   st = player.getQuestState("228_TestOfMagus")
   if st :
