# Collector of Jewels - Version 0.1 by DrLecter (adapted for L2JLisvus by roko91)
...
ADENA=57
#MOBS & DROP
DROPLIST_FREEZE={747:[FREEZING_SHARD,85], #Roxide
          619:[FREEZING_SHARD,73], #Rowin Undine
          616:[FREEZING_SHARD,60], #Undine Lakin
          }       
DROPLIST_FLARE={612:[FLARE_SHARD,77],    #Salamander Rowin
          609:[FLARE_SHARD,77],    #Salamander Lakin
          749:[FLARE_SHARD,85]     #Death Fire
          }
...
class Quest (JQuest) :
 ...
 def onEvent (self,event,st) :
   htmltext = event
   cond = st.getInt("cond")
   if event == "7376-03.htm" and cond == 0 :
     st.set("cond","1")
     st.setState(STARTED)
     st.set("awaitsFreezing","1")
     st.set("awaitsFlare","1")
     st.playSound("ItemSound.quest_accept")
   elif event == "7376-07.htm" :
     st.playSound("ItemSound.quest_itemget")
   elif event == "7376-08.htm" :
     st.exitQuest(1)
 ...
 def onTalk (self,npc,st):
   ...
   if cond == 0 :
     if st.getPlayer().getLevel() >= 25 :
       htmltext = "7376-02.htm"
     else:
       htmltext = "7376-01.htm"
       st.exitQuest(1)
   elif cond == 1 :
