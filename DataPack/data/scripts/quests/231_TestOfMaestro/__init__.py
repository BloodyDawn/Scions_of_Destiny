...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,st):
   ...
   if npcId == 7531:
     ...
     elif st.getInt("cond")>0 and st.getInt("cond")<17 :
     ...
     if (st.getInt("cond")==1 or st.getInt("cond")==11 or st.getInt("cond")==16) and st.getQuestItemsCount(RECOMMENDATION_OF_BALANKI_ID)==0:
     ...
     elif st.getInt("cond")==7 or st.getInt("cond")==17 :
   ...
   elif npcId == 7672 and st.getInt("cond")==3 :
       htmltext = "7672-01.htm"
   elif npcId == 7675 and st.getInt("cond")==3:
       st.set("cond","4")
       htmltext="7675-01.htm"
   elif npcId == 7536:
     if (st.getInt("cond")==1 or st.getInt("cond")==7 or st.getInt("cond")==16) and st.getQuestItemsCount(RECOMMENDATION_OF_ARIN_ID)==0:
       ...
       st.set("cond","11")
       recommendationCount(st)
     elif st.getInt("cond")==11 or st.getInt("cond")==17:
     ...
     if (st.getInt("cond")==1 or st.getInt("cond")==7 or st.getInt("cond")==11) and st.getQuestItemsCount(RECOMMENDATION_OF_FILAUR_ID)==0 :
   ...
   elif npcId==7532 and st.getInt("cond") :
      htmltext = "7532-01.htm"
   return htmltext

 def onKill (self,npc,player,isPet):
   st = player.getQuestState("231_TestOfMaestro")
   if st :
     if st.getState() != STARTED : return
     npcId = npc.getNpcId()
     condition,maxcount,item=DROPLIST[npcId]
     count=st.getQuestItemsCount(item)
     if st.getInt("cond") == condition and count < maxcount :
        st.giveItems(item,1)
        if count == maxcount-1 :
