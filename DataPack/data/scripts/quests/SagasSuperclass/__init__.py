...
qn = "SagasSuperclass"
Archon_Minions = range(1646,1652)
Guardian_Angels = [5214, 5215, 5216]
Archon_Hellisha_Norm = [13047, 13049, 13050, 13051, 13052]
...
class Quest (JQuest) :
 ...
 def onAdvEvent (self,event,npc, player) :
   ...
   if event == "accept" :
       ...
       st.set("Quest",str(st.getInt("Quest")+1))
       if st.getInt("Quest0") == 1 or st.getInt("Quest0") == 2 or st.getInt("Quest") > 3 :
           st.set("Quest0","0")
 ...
 def onTalk (self,npc,st):
    ...
    elif st.getPlayer().getClassId().getId() == self.prevclass :
        if cond == 0 :
            if npcId == self.NPC[0]:
                htmltext = "0-01.htm"
        elif cond == 1 :
            ...
            if npcId == self.NPC[0] :
                if st.getPlayer().getLevel() >= 76 :
                    ...
                    st.giveItems(57,5000000)
                    st.giveItems(6622,1)
                    st.getPlayer().setClassId(self.classid)
                    if not st.getPlayer().isSubClassActive() and st.getPlayer().getBaseClass() == self.prevclass :
                        st.getPlayer().setBaseClass(self.classid)
                    st.getPlayer().broadcastUserInfo()
                    self.Cast(npc,st.getPlayer(),4339,1)
                    st.exitQuest(False)
                else :
                    htmltext = "0-010.htm"
    return htmltext
    ...
    if st:
        if st.getInt("cond") == 17 :
            if npc.getNpcId() == self.Mob[2] :
                st1 = self.findRightState(player,npc)
                if st == st1 :
    ...
    if npcId in Archon_Minions :
        ...
        if st1 :
            if st1.getInt("cond") == 15 :
                ...
                self.AutoChat(npc,st1.getQuest().Text[4].replace('PLAYERNAME',player.getName()))
        ...
        if npcId == self.Mob[0] and cond == 8 :
            ...
            if st1 :
                if not player.isInParty():
                    if st == st1 :
                        ...
                        st.set("cond","9")
                        st.playSound("ItemSound.quest_middle")
                if st1.getQuestTimer("Mob_1 has despawned") :
                   st1.getQuestTimer("Mob_1 has despawned").cancel()
                self.DeleteSpawn(st1,st1.getInt("Mob_1"))
                st1.set("spawned","0")
            ...
            if cond == 15 :
                ...
                if st1 :
                    ...
                    if st1.getQuestTimer("Archon Hellisha has despawned") :
                       st1.getQuestTimer("Archon Hellisha has despawned").cancel()
                    self.DeleteSpawn(st1,st1.getInt("Archon"))
                    st1.set("spawned","0")
    else :
        if npcId == self.Mob[0] :
