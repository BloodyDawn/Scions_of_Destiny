...
class Quest (JQuest) :
 ...
 def onTalk (Self,npc,st):
   ...
   if id == CREATED :
     st.set("cond","0")
   if npcId == 7336 and st.getInt("cond")==0 :
      if st.getPlayer().getLevel() >= 15 :
         ...
         st.exitQuest(1)
   elif npcId == 7336 and st.getInt("cond") and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) == 0 :
      htmltext = "7336-04.htm"
   elif npcId == 7336 and st.getInt("cond") and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) :
      htmltext = "7336-05.htm"
   elif npcId == 7434 and st.getInt("cond") and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) == 0 :
      htmltext = "7434-01.htm"
   elif npcId == 7434 and st.getInt("cond") and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) and not pieces(st) :
      htmltext = "7434-04.htm"
   elif npcId == 7434 and st.getInt("cond") and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) and pieces(st) and not st.getQuestItemsCount(COMPLETE_SKELETON_ID):
      htmltext = "7434-05.htm"
   elif npcId == 7434 and st.getInt("cond") and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) and pieces(st) and st.getQuestItemsCount(COMPLETE_SKELETON_ID) :
      htmltext = "7434-08.htm"
   elif npcId == 7342 and st.getInt("cond") and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) :
      htmltext = "7342-01.htm"
   return htmltext

 def onKill (self,npc,player,isPet):
   st = player.getQuestState("325_GrimCollector")
   if st :
...
QUEST.addTalkId(7336)
QUEST.addTalkId(7342)
QUEST.addTalkId(7434)
