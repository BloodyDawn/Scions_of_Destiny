	...
	def onEvent (self,event,st):
		...
		if st.getInt("phase")==0:
			...
			if event == "7565_2":
				...
				st.addSpawn(7643,21036,-107690,-3038)
...
#           	end of phase 1  
	...
	def onKill (self,npc,player,isPet):
                st = player.getQuestState("232_TestOfLord")
                if st :
		   ...
		   if item == MARSH_SPIDER_FEELER_ID and st.getInt(var) == value:
			if spiderCount<10:
				...
				st.set("dudaStat","3")
		   elif st.getInt(var) == value and random < chance and count < maxcount:
			st.giveItems(item,1)
			if count == maxcount-1:
				st.playSound("ItemSound.quest_middle")
