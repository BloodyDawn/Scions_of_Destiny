...
def check_eggs(st, npc, progress) :
    ...
    if npc == "mymyu" :
       if progress in [19,20] :
          if st.getQuestItemsCount(eggs) == 1 or st.getInt("cond") == 8 :
...
class Quest (JQuest):
      ...
      if event == "ido" :
         ...
         if event == "exarion_1" :
            ...
            if st.getQuestItemsCount(SCALE_3) :
             ...
             if st.getQuestItemsCount(eggs) or st.getInt("cond") == 8 :
                st.takeItems(eggs,1)
                st.set("cond","8")
                if progress == 19 :
  ...
  def onKill (self,npc,player,isPet):
    st = player.getQuestState("420_LittleWings")
    if st :
      id   = st.getState()
      npcid = npc.getNpcId()
    #incipios drop
