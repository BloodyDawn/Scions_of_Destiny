...
class Quest (JQuest) :
 ...
 def onTalk (Self,npc,st):
   ...
   if id == CREATED :
     ...
     st.set("onlyone","0")
     st.set("id","0")
   if npcId == 7010 and st.getInt("cond")==0 :
      htmltext = "7010-01.htm"
   elif npcId == 7010 and st.getInt("cond") and st.getQuestItemsCount(EINS_LETTER_ID)>0 :
      htmltext = "7010-07.htm"
   elif npcId == 7010 and st.getInt("cond") and st.getQuestItemsCount(WARRIOR_GUILD_MARK_ID)==1 :
      htmltext = "7010-08.htm"
   elif npcId == 7253 and st.getInt("cond") and st.getQuestItemsCount(EINS_LETTER_ID) :
      htmltext = "7253-01.htm"
   elif npcId == 7253 and st.getInt("cond") and st.getQuestItemsCount(WARRIOR_GUILD_MARK_ID) :
   ...
   elif npcId == 7253 and st.getInt("cond") and st.getQuestItemsCount(SIMPLONS_LETTER_ID) :
        htmltext = "7253-06.htm"
   elif npcId == 7010 and st.getInt("cond") and st.getQuestItemsCount(SIMPLONS_LETTER_ID) and st.getQuestItemsCount(RUSTED_BRONZE_SWORD2_ID) and st.getQuestItemsCount(WARRIOR_GUILD_MARK_ID)==0 and st.getQuestItemsCount(EINS_LETTER_ID)==0 :
        htmltext = "7010-09.htm"
   elif npcId == 7010 and st.getInt("cond") and st.getQuestItemsCount(RUSTED_BRONZE_SWORD3_ID) and st.getQuestItemsCount(WARRIOR_GUILD_MARK_ID)==0 and st.getQuestItemsCount(EINS_LETTER_ID)==0 :
 ...
 def onKill (self,npc,player,isPet):
   st = player.getQuestState("401_PathToWarrior")
   if st :
      if st.getState() != STARTED : return
      npcId = npc.getNpcId()
      if npcId == 35 or npcId == 42 :
        st.set("id","0")
        if st.getInt("cond") == 2 and st.getQuestItemsCount(RUSTED_BRONZE_SWORD1_ID)<10 :
        ...
        st.set("id","0")
        if st.getInt("cond") and st.getQuestItemsCount(POISON_SPIDER_LEG2_ID)<20 and st.getQuestItemsCount(RUSTED_BRONZE_SWORD3_ID) == 1 and st.getItemEquipped(7) == RUSTED_BRONZE_SWORD3_ID:
