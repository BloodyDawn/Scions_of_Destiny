...
public abstract class L2Skill
{
    ...
    {
        return ((getSkillType() == SkillType.FATAL) || (getSkillType() == SkillType.PDAM) || (getSkillType() == SkillType.BLOW) || (getSkillType() == SkillType.CHARGEDAM));
    }
        ...
        switch (targetType)
        {
            ...
            case TARGET_ONE:
            {
                ...
                if (target.isDead()
                    || (target == activeChar && !(skillType == SkillType.BUFF
                        ...
                        || skillType == SkillType.MANARECHARGE || skillType == SkillType.SEED
                        || skillType == SkillType.REFLECT || skillType == SkillType.COMBATPOINTHEAL
                    ...
                    if (obj instanceof L2Attackable || obj instanceof L2PlayableInstance)
                    {
                        ...
                        if (src != null)
                        {
                            ...
                            if (trg != null) 
                            {
                                ...
                                if (!srcInArena && !(activeChar.isInsideZone(L2Character.ZONE_PVP) && !activeChar.isInsideZone(L2Character.ZONE_SIEGE)))
                                {
            ...
            case TARGET_FRONT_AREA:
            {
                ...
                if ((!(target instanceof L2Attackable || target instanceof L2PlayableInstance)) ||  //   Target is not L2Attackable or L2PlayableInstance
                    (getCastRange() >= 0 && (target == activeChar || target.isDead()))) // target is null or self or dead/faking
                {
                    activeChar.sendPacket(new SystemMessage(SystemMessage.TARGET_IS_INCORRECT));
                    return null;
                }
            ...
            case TARGET_PARTY_MEMBER:
            {
                ...
                if (target == activeChar || (activeChar.getParty() != null && target.getParty() != null
                            ...
                            if (((L2PcInstance) newTarget).getPet() != null)
                            {
                                if (Util.checkIfInRange(radius, activeChar, ((L2PcInstance) newTarget).getPet(), true))
                                {
                                    if ((targetType != SkillTargetType.TARGET_CORPSE_ALLY)
                                        ...
                                        && player.checkPvpSkill(newTarget, this) && onlyFirst == false)
            ...
            case TARGET_CORPSE_PLAYER:
            {
                ...
                if (getSkillType() == SkillType.RESURRECT)
                {
                    ...
                    if (target instanceof L2PcInstance)
                        dead = (L2PcInstance)target;
                    else if (target instanceof L2PetInstance)
                        dead = (L2PetInstance)target;
            ...
            case TARGET_AREA_CORPSE_MOB:
            {
                ...
                for (L2Character obj : activeChar.getKnownList().getKnownCharacters())
                {
                    if (!(obj instanceof L2Attackable || obj instanceof L2PlayableInstance) || obj.isDead()
                           || obj == activeChar)
                        continue;
    ...
    public final Func[] getStatFuncs(L2Effect effect, L2Character player)
    {
        if (!(player instanceof L2PcInstance)
                ...
                && !(player instanceof L2SiegeFlagInstance))
