...
public class TvTEvent
{
    ...
    public static void initialize(Properties settings)
    {
        ...
        if (!isEnabled && _state == 0) // Default state
    ...
    private void prepareColiseum()
    {
        if (_arena != null)
        {
            for (L2Character activeChar : _arena._characterList.values())
            {
    ...
    private void eventRemovals()
    {
        ...
        if (_blueTeamKills > _redTeamKills)
        {
            for (L2PcInstance blue : _blueTeam)
            {
                ...
                for (L2ItemInstance reward : _rewardList)
                {
                    if (reward == null)
                        continue;
    ...
    public static void registerPlayer(L2PcInstance player)
    {
        ...
        for (L2PcInstance registered : _registered)
        {
            ...
            if (registered.getNetConnection() == null || player.getNetConnection() == null)
                continue;

            String ip1 = player.getNetConnection().getIP();
            String ip2 = registered.getNetConnection().getIP();
