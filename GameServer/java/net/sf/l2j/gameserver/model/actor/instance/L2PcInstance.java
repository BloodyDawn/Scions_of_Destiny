...
import net.sf.l2j.gameserver.serverpackets.ExOlympiadMatchEnd;
...
public final class L2PcInstance extends L2PlayableInstance
{
    ...
    private MMOConnection _connection;
    ...
    // Friend list
    private List<Friend> _friendList = new FastList<>();
    ...
    {
        if (_connection != null && _connection.getClient().getLoginName() != null)
     ...
     *
     * @param getNewbieState The Identifier of the _newbieState state<BR><BR>
     *
     */
    ...
    public void destroyWearedItems(String process, L2Object reference, boolean sendMessage)
    {
        ...
        ItemList il = new ItemList(getInventory().getItemList(), true);
        sendPacket(il);
     ...
     */
    public MMOConnection getNetConnection()
    {
        return _connection;
    }
     ...
     */
    public void setNetConnection(MMOConnection connection)
    {
    ...
    {
        MMOConnection con = getNetConnection();
        if (con != null)
        {
            con.close(false);
            con.sendPacket(new LeaveWorld());
        }
    }
	...
	if (effects != null && effects.length > 0)
        ...
        if (os != null)
        {
            ...
            {
                for (L2PcInstance spectator : Olympiad.getInstance().getSpectators(getOlympiadGameId()))
                {
                    if (spectator == null)
                        continue;
    ...
    public void broadcastPacket(ServerBasePacket mov, int radiusInKnownlist)
    {
        if (!(mov instanceof CharInfo))
            sendPacket(mov);
    ...
    protected void doPickupItem(L2Object object)
    {
        ...
        synchronized (target)
        {
            ...
            if (target.getItemLootSchedule() != null && (target.getOwnerId() == getObjectId() || isInLooterParty(target.getOwnerId())))
    ...
    /**
     * Manage the increase level task of a L2PcInstance (Max MP, Max MP, recommendation, Expertise and beginner skills...).<BR><BR>
     *
     * <B><U> Actions</U> :</B><BR><BR>
    ...
    /**
     * Set the _clan object, _clanId, Flag and title of the L2PcInstance.<BR><BR>
     */
    ...
    /**
     * Store dwarven recipe book data for this L2PcInstance, if not on an active sub-class.
     */
    private void storeDwarvenRecipeBook()
    {
        if (getDwarvenRecipeBook().length == 0)
            return;

        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_recipebook WHERE char_id=? AND type=1"))
            {
                statement.setInt(1, getObjectId());
                statement.execute();
            }
        ...
        {
            _log.warning("Could not store dwarven recipe book data: " + e);
        }
    }

    /**
     * Store common recipe book data for this L2PcInstance.
     */
    private void storeCommonRecipeBook()
    {
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_recipebook WHERE char_id=? AND type=0"))
            {
                statement.setInt(1, getObjectId());
                statement.execute();
            }
            ...
            for (int count = 0; count < recipes.length; count++)
            {
                try (PreparedStatement statement = con.prepareStatement("INSERT INTO character_recipebook (char_id, id, type) values(?,?,0)"))
                {
                    statement.setInt(1, getObjectId());
                    statement.setInt(2, recipes[count].getId());
                    statement.execute();
        ...
        {
            _log.warning("Could not store common recipe book data: " + e);
        }
    }

    /**
     * Restore recipe book data for this L2PcInstance.
     * recipeType is the type you do not want to restore.
     */
    private void restoreRecipeBook(int recipeType)
    {
        try (Connection con = L2DatabaseFactory.getInstance().getConnection();
            PreparedStatement statement = con.prepareStatement("SELECT id, type FROM character_recipebook WHERE char_id=? AND type<>?"))
        {
            statement.setInt(1, getObjectId());
            statement.setInt(2, recipeType);
            try (ResultSet rset = statement.executeQuery())
            {
                L2RecipeList recipe;
                while (rset.next())
            ...
            PreparedStatement statement = con.prepareStatement(ADD_SKILL_SAVE))
        {
            ...
            if (storeEffects)
            {
                for (L2Effect effect : getAllEffects())
                {
                    ...
                    if (effect != null && effect.getInUse() && !skill.isToggle())
                    {
                        buff_index++;
    ...
    private void notifyFriends()
    {
        ...
        for (Friend friend : getFriendList())
        {
            ...
            {
                L2PcInstance friendChar = L2World.getInstance().getPlayer(friend.getName());
                if (friendChar == null)
                    return;
    ...
    public boolean isAutoAttackable(L2Character attacker)
    {
        ...
        if (player != null)
        {
            ...
            if (getClan() != null)
            {
                ...
                if (getClan() != null
                 && player.getClan() != null
                 && (getClan().isAtWarWith(player.getClanId())
                 && player.getClan().isAtWarWith(getClanId())
                 && getWantsPeace() == 0
               ...
               !targetPlayer.isInsideZone(ZONE_PVP))  // target is not in PvP zone
        {
            if (skill.isPvpSkill()) // pvp skill
            {
                ...
                if (targetPlayer.getPvpFlag() == 0 &&        //   target's pvp flag is not set and
                       targetPlayer.getKarma() == 0)         //   target has no karma
                    return false;
            }
    ...
    {
        L2CubicInstance cubic = new L2CubicInstance(this, id, level, givenByOther);
    ...
    public void deleteMe()
    {
        ...
        try
        {
            if (Olympiad.getInstance().isRegisteredInComp(this)) // handle removal from olympiad game
    ...
    {
        return _connection.getClient().getFloodProtectors();
    }
