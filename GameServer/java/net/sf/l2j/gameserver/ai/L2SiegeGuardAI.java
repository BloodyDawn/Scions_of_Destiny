...
public class L2SiegeGuardAI extends L2CharacterAI implements Runnable
{
    ...
    private void thinkActive()
    {
        ...
        if (_globalAggro >= 0)
        {
            ...
            L2Character hated;
            if (_actor.isConfused()) hated = _attack_target; // Force mobs to attack anybody if confused
    ...
    private final void factionNotifyAndSupport()
    {
        ...
        for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
        {
            ...
            if (npc.getAI() != null) // TODO: possibly check not needed
            {
                ...
                // heal friends
                if (_selfAnalysis.hasHealOrResurrect && !_actor.isAttackingDisabled() && npc.getCurrentHp() < npc.getMaxHp()*0.6
    ...
    private void attackPrepare()
    {
        ...
        if (!_actor.isMuted() && dist_2 > range * range)
        {
            ...
            else
            {
                ...
                double homeX = _attack_target.getX() - sGuard.getSpawn().getLocx();
                double homeY = _attack_target.getY() - sGuard.getSpawn().getLocy();
