...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	private void showEditDropData(L2PcInstance admin, int npcId, int itemId, int category)
	{
	        ...
	        NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
	        TextBuilder replyMSG = new TextBuilder("<html><title>Details of Drop Data: (" + npcId + " " + itemId + " " + category + ")</title>");
	        replyMSG.append("<body>");
    ...
    private void reLoadNpcDropList(int npcId)
    {
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            ...
            try (PreparedStatement statement = con.prepareStatement("SELECT " + L2DatabaseFactory.getInstance().safetyString(new String[] {"mobId", "itemId", "min", "max", "category", "chance"}) + " FROM droplist WHERE mobId=?"))
            {
                statement.setInt(1, npcId);
            ...
            if (Config.CUSTOM_DROPLIST_TABLE)
            {
                try (PreparedStatement statement = con.prepareStatement("SELECT " + L2DatabaseFactory.getInstance().safetyString(new String[] {"mobId", "itemId", "min", "max", "category", "chance"}) + " FROM custom_droplist WHERE mobId=?"))
                {
                    statement.setInt(1, npcId);
