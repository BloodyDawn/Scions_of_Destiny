...
public abstract class ServerBasePacket extends BasePacket
{
    ...
    protected ServerBasePacket(MMOConnection con)
    {
        super(con);
        if (Config.DEBUG)
            _log.fine(getType());
    ...
    public final BasePacket setConnection(MMOConnection con)
    {
    ...
    public final boolean write(ByteBuffer buf)
    {
        ...
        try
        {
            ...
            {
                if (Config.DEBUG)
                    _log.finer(getType()+" >>> "+(getClient() == null?"null":getClient().getLoginName()));
