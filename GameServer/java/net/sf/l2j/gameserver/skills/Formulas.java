...
public final class Formulas
{
	...
	static class FuncPDefMod extends Func
	{
		...
		public void calc(Env env)
		{
			if (env.player instanceof L2PcInstance)
                        {
                                L2PcInstance p = (L2PcInstance) env.player;
                                boolean hasMagePDef = (p.getClassId().isMage() || p.getClassId().getId() == 0x31);
    			        ...
    			        if (p.getInventory().getPaperdollItem(Inventory.PAPERDOLL_LEGS) != null || (p.getInventory().getPaperdollItem(Inventory.PAPERDOLL_CHEST) != null
                                && p.getInventory().getPaperdollItem(Inventory.PAPERDOLL_CHEST).getItem().getBodyPart() == L2Item.SLOT_FULL_ARMOR))
	...
	public final int calcMAtkSpd(L2Character attacker, @SuppressWarnings("unused") L2Character target, L2Skill skill, double skillTime)
	{
		if (skill.isMagic())
		{
	...
	public boolean calcSkillSuccess(L2Character attacker, L2Character target, L2Skill skill, boolean ss, boolean sps, boolean bss)
		...
		double statmodifier = calcSkillStatModifier(type, skill.getSaveVs(), target);
